...
if ! kill -0 "$AGENT_PID" 2>/dev/null; then
  ...
  while kill -0 "$AGENT_PID" 2>/dev/null; do
    ...
    if [ "$WATCH_RC" -ne 0 ]; then
      # A single transport timeout/reset must not tear down an otherwise healthy paid session.
      # The independent local hard-deadline guard remains authoritative while we make exactly
      # one bounded retry. HTTP rejections and malformed responses are never retried.
      echo "WATCH TRANSPORT RETRY: προσωρινή αποτυχία επικοινωνίας (curl rc=${WATCH_RC}); μία ασφαλής επανάληψη σε 2s."
